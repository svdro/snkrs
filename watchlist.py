...
def should_notify(
    ...
    for p in watched_products:
        ...
        if p.id in all_changes["availability"] or all_changes["add"]:
            ...
            has_become_available_but_restricted = False
            if info.get("include_restricted"):
                prev_restricted = queries.is_available(
                    p, -2, sizes=info.get("sizes", []), exclusive_access=True
                )
                curr_restricted = queries.is_available(
                    p, -1, sizes=info.get("sizes", []), exclusive_access=True
                )
                has_become_available_but_restricted = (
                    not prev_restricted and curr_restricted
                )

            has_become_available = not prev_available and curr_available

            # if not prev_available and curr_available:
            if has_become_available or has_become_available_but_restricted:
                notify.append(p)
